...
namespace ExpandedChestUI.Scripts.Components
{
    public class ExpandedInventorySlotUI : InventorySlotUI
    {
        ...
        private bool showHoverWindow => IsMouseWithinScrollArea() && slotsUIContainer is not null && slotsUIContainer.scrollWindow.IsShowingPosition(localScrollPosition, background.size.y);
        ...
        private bool IsMouseWithinScrollArea()
        {
            ...
            return new Rect(new Vector2(position2.x, position2.y) + uiScrollWindow.windowLocalCenter - vector2, size).Contains(position1);
        }
    }
